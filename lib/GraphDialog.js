...
class GraphDialog extends events.EventEmitter {
    ...
    stepInteractionHandler(session, results, next) {
        ...
        switch (currentNode.type) {
            ...
            case Node_1.NodeType.prompt:
                ...
                builder.Prompts[promptType](session, skipAfterError ? {} : this.replaceVariables(currentNode.data.text, session), currentNode.data.options, {
    ...
    stepResultCollectionHandler(session, results, next) {
        ...
        if (results.assignments instanceof Object) {
            for (let key in results.assignments) {
                if (results.assignments.hasOwnProperty(key)) {
                    ...
                    Logging_1.Log('assigning request for node: %s, variable: %s, value: %s', currentNode.id, key, session.dialogData.data[key]);
                }
            }
        }
        ...
        Logging_1.Log('collecting response for node: %s, variable: %s, value: %s', currentNode.id, varname, session.dialogData.data[varname]);
        for (let additionalVarname of currentNode.additionalVarnames) {
            ...
            Logging_1.Log('collecting response for node: %s, variable: %s, value: %s', currentNode.id, additionalVarname, session.dialogData.data[varname]);
        }
        return next(results);
    }
