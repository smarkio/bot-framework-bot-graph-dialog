            ...
            if (nodeItem.data.js) {
                ...
                return new Promise((resolve, reject) => {
                    this.options.loadHandler(handler)
                        ...
                        this.handlers.add(handler, func);
                        console.log('node:', nodeItem.id, nodeItem._parent && nodeItem._parent.id ? '[parent: ' + nodeItem._parent.id + ']' : '', nodeItem._next && nodeItem._next.id ? '[next: ' + nodeItem._next.id + ']' : '', nodeItem._prev && nodeItem._prev.id ? '[prev: ' + nodeItem._prev.id + ']' : '', nodeItem.type ? '[type: ' + nodeItem.type + ']' : '', nodeItem.typeName ? '[typeName: ' + nodeItem.type + ']' : '');
                        return this.recursive(nodeItem).then(() => {
                            return resolve();
                        }).catch(e => reject(e));
                    }).catch(e => reject(e));
